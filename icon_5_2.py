...
while True:
    ...
    spectrum_bar_height = 40  # 스펙트럼 바 높이
    brightness_bar_height = 40  # 밝기 스펙트럼 바 높이
    ...
    # 스펙트럼 바의 Y 시작점 계산 (가장 아래에서부터 위로 쌓아 올림)
    # 1. 색상 정보 텍스트 Y 위치
    color_text_y_pos_bottom = h - padding  # 화면 가장 아래에 텍스트를 배치
    color_text_y_pos_top = color_text_y_pos_bottom - text_size_color[1] - 5  # 텍스트 박스 상단 여유분

    # 2. 색상 스펙트럼 바 Y 위치 (색상 정보 텍스트 위)
    spectrum_y_start = color_text_y_pos_top - item_spacing - spectrum_bar_height  # 스펙트럼 바 상단

    # 3. 밝기 스펙트럼 바 Y 위치 (색상 스펙트럼 바 위)
    brightness_y_start = spectrum_y_start - item_spacing - brightness_bar_height  # 밝기 스펙트럼 바 상단

    # 색상 이름 배경 박스
    cv2.rectangle(display_frame,
                  (padding - 5, color_text_y_pos_top - 5),  # 좌상단
                  (padding + text_size_color[0] + 5, color_text_y_pos_bottom + 5),  # 우하단
                  (0, 0, 0), -1)  # 검은색 배경
    ...
    sample_box_y = color_text_y_pos_top  # 텍스트의 상단과 맞춤
    cv2.rectangle(display_frame,
    ...
    hue_val_from_cv = hsv_original_center_color[0]  # Hue (0-179)
    sat_val_from_cv = hsv_original_center_color[1]  # Saturation (0-255)
    val_val_from_cv = hsv_original_center_color[2]  # Value/Brightness (0-255)
    ...
    for i in range(spectrum_bar_width):
        # 밝기 (Value)를 0에서 255까지 변화시키면서 회색조 생성
        # BGR (0,0,0) - 검은색, (255,255,255) - 흰색
        gray_val = int(i * 255 / spectrum_bar_width)  # 0-255
        brightness_spectrum[:, i] = (gray_val, gray_val, gray_val)  # 모든 채널에 동일 값 적용
    ...
    cv2.rectangle(display_frame,
                  ...
                  (200, 200, 200), 1)  # 밝기 스펙트럼 바 테두리

    # 밝기 스펙트럼 막대에 하이라이트 그리기 (현재 밝기 위치 표시)
    # Value 값 (0-255)을 스펙트럼 바 너비에 매핑
    ...
    brightness_pos_on_bar = max(padding, min(brightness_pos_on_bar, w - padding - 1))  # 화면 경계 내로 제한

    cv2.line(display_frame, (brightness_pos_on_bar, brightness_y_start),
             (brightness_pos_on_bar, brightness_y_start + brightness_bar_height), (255, 255, 255), 2)  # 하이라이트 라인
