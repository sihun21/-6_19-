...
# 빨간색 버튼을 직접 그리기 위한 플레이스홀더 생성
red_button_icon = np.zeros((icon_size[1], icon_size[0], 3), dtype=np.uint8)
red_button_icon[:] = (0, 0, 200)  # 어두운 빨간색 (BGR)
cv2.putText(red_button_icon, "R", (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

# 파란색 버튼을 직접 그리기 위한 플레이스홀더 생성
blue_button_icon = np.zeros((icon_size[1], icon_size[0], 3), dtype=np.uint8)
blue_button_icon[:] = (200, 0, 0)  # 어두운 파란색 (BGR)
cv2.putText(blue_button_icon, "B", (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
